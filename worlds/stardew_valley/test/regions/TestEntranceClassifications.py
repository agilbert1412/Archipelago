from ... import options
from ...data.regions.model import GroupFlag, RandomizationFlag
from ...data.regions.vanilla_data import connections_without_ginger_island_by_name
...
class TestSameTypeEntranceRandomization(EntranceRandomizationAssertMixin, SVTestBase):
    ...
    def test_outside_to_inside_is_conserved(self):
        ...
        self.connections_go_to_same_type(relevant_group, outside_to_inside_entrances)

    def test_inside_to_outside_is_conserved(self):
        relevant_group = GroupFlag.IN_TO_OUT
        inside_to_outside_entrances = [
        ...
        self.connections_go_to_same_type(relevant_group, inside_to_outside_entrances)

    def test_inside_to_inside_is_conserved(self):
        relevant_group = GroupFlag.IN_TO_IN
        inside_to_inside_entrances = [
        ...
        self.connections_go_to_same_type(relevant_group, inside_to_inside_entrances)

    def connections_go_to_same_type(self, relevant_group, entrances_to_test):
        mapping = {}
        mapping.update(connections_without_ginger_island_by_name)
        mapping.update(
            {
                connection.name: connection
                for connection in region_data_by_content_pack[ModNames.ginger_island].connections
            }
        )
        ...
        for entrance_name in entrances_to_test:
            with self.subTest(f"{entrance_name} is still {relevant_group.name}"):
                ...
                self.assertIn(
                    ...
                    f"[{entrance_name}] does not have the group {relevant_group.name}",
                )
                ...
                self.assertIn(
                    relevant_group,
                    destination_connection_data.group,
                    f"[{entrance_name}: {destination_entrance_name}] does not maintain the group {relevant_group.name}",
                )
